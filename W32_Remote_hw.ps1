...
$Disks          =       (Get-CimInstance Win32_DiskDrive | ForEach-Object {"$($_.Model) ($([math]::Round($_.Size / 1GB, 2)) GB)"}) -join "; "
...
$Graphics_Card  =       (Get-CimInstance Win32_VideoController | Select-Object -ExpandProperty Name) -join ", "
...
$NetworkAdapters =      (Get-NetAdapter | Where-Object { $_.MacAddress} |
                               ForEach-Object { "$($_.Name)" }) -join ", "
$MacAddress     = (Get-NetAdapter | Where-Object { $_.MacAddress } | ForEach-Object { "$($_.MacAddress)" }) -join ", "
#OEM Information
$licenseinfo2           = Get-CimInstance SoftwareLicensingProduct -Filter "Name LIKE 'Windows%' and ProductKeyId2 !=NULL" | 
                          Select-Object Name, Description, PartialProductKey, ProductKeyChannel, 
                          LicenseStatus,ProductKeyID,ProductKeyID2
$LicenseName2           = if ($licenseinfo2) { $licenseinfo2.Name } else { "N/A" }
$LicenseDesc            = if ($licenseinfo2) { $licenseinfo2.Description } else { "N/A" }
$LicenseProductKeyID2   = if ($licenseinfo2) { $licenseinfo2.ProductKeyID2 } else { "N/A" }
$licenseserialkey       = (Get-CimInstance SoftwareLicensingService)| Select-Object OA3xOriginalProductKey
$Product_Key            = wmic path softwarelicensingservice get OA3xOriginalProductKey
#
    $ComputerInfo = @{
        "HostName"                   = $cs.Caption
        "Status"                     = "OK"
