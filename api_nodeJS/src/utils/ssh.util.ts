    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new AppError(
          ...
          `SSH connect timed out to ${host}:${port}`,
        ),
      );
    }, timeoutMs);
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new AppError(
            504,
            ErrorCode.PS_TIMEOUT,
            `SSH command timed out: ${command.slice(0, 60)}`,
          ),
        );
      }, timeoutMs);
...
export function parseSystemInfo(raw: string): Partial<HardwareInfo> {
  ...
  // Memory — "16,050 MB" → GB
  const memMB = parseFloat(
    totalMemStr.replace(/,/g, "").replace(/\s*MB.*/i, ""),
  );
...
export function parseIpConfig(raw: string): NetworkAdapterInfo[] {
  ...
  for (const block of blocks) {
    ...
    const macMatch =
      /Physical Address[\s.]+:\s*([A-F0-9]{2}(?:[:-][A-F0-9]{2}){5})/i.exec(
        block,
      );
    const mac = macMatch ? macMatch[1].toUpperCase() : "";

    const ipMatches = [...block.matchAll(/IPv4 Address[\s.]+:\s*([0-9.]+)/gi)];
    const ipAddresses = ipMatches.map((m) =>
      m[1].replace(/\(Preferred\)/i, "").trim(),
    );
...
// ─── generic wmic csv parser ────────────────────────────────────────────────

export function parseWmicCsvRows(raw: string): Array<Record<string, string>> {
  const lines = raw
    .split(/\r?\n/)
    .map((l) => l.trim())
    .filter((l) => l.length > 0);
